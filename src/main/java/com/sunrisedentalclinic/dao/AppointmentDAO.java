...
public class AppointmentDAO implements IDAO<Appointment> {
    ...
    public void save(Appointment appointment) {
        String sql = "INSERT INTO appointment (appointmentNo, patientID, dentistID, treatmentID, staffID, appointmentDate, appointmentTime, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Appointment findByDentistAndDateTime(String dentistID, LocalDate date, LocalTime time) {
        String sql = "SELECT * FROM appointment WHERE dentistID=? AND appointmentDate=? AND appointmentTime=? AND status='SCHEDULED'";
        try (Connection conn = DBConnectionManager.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Appointment> findByPatient(String patientID) {
        ...
        try (Connection conn = DBConnectionManager.getInstance().getConnection();
            ...
            throw new RuntimeException("Error finding appointments for patient", e);
        }
        return appointments;
    }
    ...
    public void update(Appointment appointment) {
        String sql = "UPDATE appointment SET status=? WHERE appointmentNo=?";
        try (Connection conn = DBConnectionManager.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private Appointment mapRow(ResultSet rs) throws SQLException {
        ...
        appointment.updateStatusFromDB(AppointmentStatus.valueOf(rs.getString("status")));
        return appointment;
    }
}
