...
public class AppointmentApiServlet extends HttpServlet {
    ...
    public AppointmentApiServlet() {
        this(ServiceFactoryApi.getClinicFacade());
    }
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (!ApiSessionUtil.hasRole(request, "RECEPTIONIST")) {
            JsonUtil.writeJson(response, 403, new ApiError("Forbidden")); return;
        }
        String appointmentNo = request.getParameter("appointmentNo");
        try {
            clinicFacade.cancelAppointment(appointmentNo);
            JsonUtil.writeJson(response, 200, java.util.Map.of("status", "cancelled"));
        } catch (Exception e) {
            JsonUtil.writeJson(response, 400, new ApiError(e.getMessage()));
        }
    }
}